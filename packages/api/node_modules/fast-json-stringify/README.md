...
__Note__: In the case of a string formatted Date and not Date Object, there will be no manipulation on it. It should be properly formatted.
...
`fast-json-stringify` supports the __anyOf__ and __oneOf__ keywords as defined by JSON schema. Both must be an array of valid JSON schemas. The different schemas will be tested in the specified order. The more schemas `stringify` has to try before finding a match, the slower it will be.
...
__NB__ Do not declare the properties twice or you will print them twice!
...
possible performance gain. With this option set, __large arrays__ would be
...
within __large arrays__ completely. By doing so the overhead previously
mentioned is nulled, greatly improving execution time. Mind there's no change
in behavior for arrays not considered *large*
