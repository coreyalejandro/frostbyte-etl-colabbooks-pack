...
Output: `docs/TENANT_ISOLATION_STORAGE_ENCRYPTION.md` -- a standalone architecture specification document covering ISOL-02 (per-tenant storage namespaces) and ISOL-03 (encryption key management).
</objective>
...
@docs/PRD.md (Section 3.4: Provisioning Actions Detail steps 4-7, Section 3.6: Deprovisioning steps 3-5)
@docs/TECH_DECISIONS.md (Component #11: PostgreSQL >=16, #15: Qdrant >=1.13, #16: MinIO, #19: Redis >=8.0, #20: SOPS + age)
...
<task type="auto">
  ...
  <files>docs/TENANT_ISOLATION_STORAGE_ENCRYPTION.md</files>
  <action>
Create `docs/TENANT_ISOLATION_STORAGE_ENCRYPTION.md` with the following structure and content:
...
1. `grep -c "MinIO\|minio\|mc " docs/TENANT_ISOLATION_STORAGE_ENCRYPTION.md` returns >= 15
2. `grep -c "PostgreSQL\|psql\|CREATE DATABASE\|pg_hba" docs/TENANT_ISOLATION_STORAGE_ENCRYPTION.md` returns >= 15
3. `grep -c "Qdrant\|qdrant\|collection" docs/TENANT_ISOLATION_STORAGE_ENCRYPTION.md` returns >= 15
4. `grep -c "Redis\|redis\|ACL" docs/TENANT_ISOLATION_STORAGE_ENCRYPTION.md` returns >= 10
5. `grep -c "tenant_id\|tenant_{" docs/TENANT_ISOLATION_STORAGE_ENCRYPTION.md` returns >= 30
6. `grep -c "verification\|Verification\|verify\|Verify" docs/TENANT_ISOLATION_STORAGE_ENCRYPTION.md` returns >= 10
7. `wc -l docs/TENANT_ISOLATION_STORAGE_ENCRYPTION.md` returns >= 350
  </verify>
...
<task type="auto">
  ...
  <files>docs/TENANT_ISOLATION_STORAGE_ENCRYPTION.md</files>
  <action>
Add the following sections to the existing `docs/TENANT_ISOLATION_STORAGE_ENCRYPTION.md`:
...
1. `grep -c "age-keygen\|SOPS\|sops\|age1\|envelope" docs/TENANT_ISOLATION_STORAGE_ENCRYPTION.md` returns >= 15 (encryption lifecycle covered)
2. `grep -c "rotate\|rotation\|Rotation" docs/TENANT_ISOLATION_STORAGE_ENCRYPTION.md` returns >= 8 (rotation procedure detailed)
3. `grep -c "KEK\|DEK\|AES256_GCM\|key hierarchy\|envelope encryption" docs/TENANT_ISOLATION_STORAGE_ENCRYPTION.md` returns >= 5 (three-tier hierarchy documented)
4. `grep -c "backup\|archive\|archived\|recovery" docs/TENANT_ISOLATION_STORAGE_ENCRYPTION.md` returns >= 5 (backup safety addressed)
5. `grep -c "tenant_key_history\|age_public_key\|key_version" docs/TENANT_ISOLATION_STORAGE_ENCRYPTION.md` returns >= 3 (registry schema exists)
6. `grep -c "PRD.*3\\.4\|PRD.*3\\.6\|Section 3" docs/TENANT_ISOLATION_STORAGE_ENCRYPTION.md` returns >= 3 (PRD cross-references)
7. `wc -l docs/TENANT_ISOLATION_STORAGE_ENCRYPTION.md` returns >= 500 (total document with both tasks)
8. `grep -c "mermaid\|```" docs/TENANT_ISOLATION_STORAGE_ENCRYPTION.md` returns >= 10 (code blocks and diagrams present)
  </verify>
...
- docs/TENANT_ISOLATION_STORAGE_ENCRYPTION.md exists with >= 500 lines
