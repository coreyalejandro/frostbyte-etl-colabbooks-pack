...
must_haves:
  ...
  artifacts:
    - path: "docs/POLICY_ENGINE_PLAN.md"
...
<context>
@docs/PRD.md (Section 2.3 Phase C)
@docs/DOCUMENT_SAFETY.md (Section 1: injection patterns, scorer, decision tree)
@docs/AUDIT_ARCHITECTURE.md (Section 1.2: POLICY_GATE_*, DOCUMENT_QUARANTINED, PII_DETECTED)
@docs/PARSING_PIPELINE_PLAN.md (canonical JSON, chunk schema)
</context>

<tasks>
<task type="auto">
  <name>Write policy engine implementation plan</name>
  <files>docs/POLICY_ENGINE_PLAN.md</files>
  <action>
Create docs/POLICY_ENGINE_PLAN.md with: (1) Overview — three gates run before embedding; (2) Gate 1 PII: NER scan, tenant actions REDACT/FLAG/BLOCK, PII types from PRD Appendix G; (3) Gate 2 Classification: rule-based then ML, categories (contract, invoice, SOP, etc.), review queue routing; (4) Gate 3 Injection: DOCUMENT_SAFETY Section 1 patterns, heuristic scorer, PASS/FLAG/QUARANTINE decision; (5) Chunk ID confirmation (deterministic from PARSING_PIPELINE_PLAN); (6) Policy-enriched chunk schema (Pydantic); (7) Audit events; (8) Online/offline mode (policy logic identical; classifiers may differ)
  </action>
</task>
</tasks>
