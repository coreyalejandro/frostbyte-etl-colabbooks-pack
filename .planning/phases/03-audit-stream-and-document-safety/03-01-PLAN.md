...
Output: `docs/AUDIT_ARCHITECTURE.md` — a standalone architecture specification covering AUDIT-01, AUDIT-02, AUDIT-03.
</objective>
...
@docs/PRD.md (Section 2 audit schema, Appendix A event types, Section 5.7 Audit API)
...
<task type="auto">
  ...
  <files>docs/AUDIT_ARCHITECTURE.md</files>
  <action>
Create `docs/AUDIT_ARCHITECTURE.md` with the following structure:
...
- Export format: JSON Lines, one event per line, with manifest containing SHA-256 of export file
  ...
  <verify>
1. grep -c "event_id\|tenant_id\|event_type\|previous_event_id" docs/AUDIT_ARCHITECTURE.md >= 15
2. grep -c "CREATE TABLE\|REVOKE\|GRANT\|INSERT" docs/AUDIT_ARCHITECTURE.md >= 5
3. grep -c "mermaid" docs/AUDIT_ARCHITECTURE.md >= 1
4. wc -l docs/AUDIT_ARCHITECTURE.md >= 250
  </verify>
  <done>
...
<task type="auto">
  ...
  <files>docs/AUDIT_ARCHITECTURE.md</files>
  <action>
Add the following sections to the existing `docs/AUDIT_ARCHITECTURE.md`:
...
- Rate limits: 300 req/min per tenant for audit API (from PRD)
  ...
  <verify>
1. grep -c "BY tenant\|BY document\|BY time\|workflow\|Workflow" docs/AUDIT_ARCHITECTURE.md >= 5
2. grep -c "SELECT\|FROM audit\|WHERE" docs/AUDIT_ARCHITECTURE.md >= 8
3. grep -c "export\|Export\|jsonl\|JSON Lines" docs/AUDIT_ARCHITECTURE.md >= 5
4. wc -l docs/AUDIT_ARCHITECTURE.md >= 400
  </verify>
  <done>
...
<success_criteria>
- docs/AUDIT_ARCHITECTURE.md exists with >= 400 lines
