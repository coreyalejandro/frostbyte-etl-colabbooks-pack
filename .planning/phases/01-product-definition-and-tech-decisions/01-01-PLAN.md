...
- Do not include forward references to documents that do not yet exist.
  ...
  <verify>
Verify the PRD satisfies all 5 success criteria from the roadmap:

1. Read docs/PRD.md and confirm: Can you describe the full pipeline lifecycle (intake through serving) from the PRD alone? Are all 4 personas identified with their roles? Are monitoring requirements specified?

2. Count the data flow diagrams — there must be exactly 5 (one per pipeline phase: intake, parsing, policy, embedding/storage, serving). Each must show typed inputs and outputs.

3. Verify the tenant lifecycle state machine: Are all 7 states present (PENDING, PROVISIONING, ACTIVE, SUSPENDED, DEPROVISIONING, DEPROVISIONED, FAILED)? Is there a transition table with triggers, pre-conditions, actions, audit events, and rollback for every transition?

4. Verify API contracts: Does every endpoint have HTTP method, path, request schema, response schema, error responses, auth requirements, and rate limits? Check that the error envelope is consistent.

5. Verify monitoring: Are concrete metric names defined (not just "use Prometheus")? Are alert conditions specified with severity and thresholds? Are log requirements specified with required fields?

6. Verify no implementation details leaked in: No library version numbers, no Docker configurations, no specific tool names in the pipeline phase descriptions (except in the glossary where the tech context is established).

7. Verify the glossary exists and defines all key terms.

8. Verify the anti-features table is present in Section 1.
  </verify>
  <done>
